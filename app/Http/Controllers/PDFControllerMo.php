...
class PDFControllerMo extends Controller
{
    ...
    {
        $record = MediaOrder::with('quotationM', 'quotationM.clientsO', 'quotationM.brandsO', 'quotationM.projectO', 'quotationM.oQuote')->find($record->id);

        $data = [
            'title' => 'Media Order',
            'name' => $record->quotationM->clientsO->client_name,
            'client_slug' => $record->quotationM->clientsO->client_slug,
            'brand_slug' => $record->quotationM->brandsO->brand_slug,
            ...
            'medias' => $record->quotationM->projectO->project_name,
            'total' => 0,
            // 'no' => $items->id,
